...
{
    public class SRTPluginUIRER1DirectXOverlay : PluginBase, IPluginUI
    {
        internal static PluginInfo _Info = new PluginInfo();
        public override IPluginInfo Info => _Info;
        public string RequiredProvider => "SRTPluginProviderRER1";
        private IPluginHostDelegates hostDelegates;
        private IGameMemoryRER1 gameMemory;
        ...
        private Process GetProcess() => Process.GetProcessesByName("rerev")?.FirstOrDefault();
        ...
        {
            this.gameMemory = (IGameMemoryRER1)gameMemory;
        ...
        private void DrawOverlay()
        {
            ...
            {
                DrawHealthBar(ref statsXOffset, ref statsYOffset, gameMemory.Player.Name, gameMemory.Player.CurrentHP, gameMemory.Player.MaxHP, gameMemory.Player.Percentage);
            }
            else
            {
                ...
                _graphics?.DrawText(_consolasBold, 20f, TextColor, statsXOffset + 10f, statsYOffset += 24, string.Format("{0}{1} / {2} {3:P1}", gameMemory.Player.Name, gameMemory.Player.CurrentHP, gameMemory.Player.MaxHP, perc));
            }
            ...
            if (config.ShowIGT)
            {
                ...
                textOffsetX = 0f;
                _graphics?.DrawText(_consolasBold, 20f, _grey, config.PositionX + 15f, statsYOffset += 24, config.SegmentString); //178f
                textOffsetX += config.PositionX + 15f + GetStringSize(config.SegmentString) + 10f;
                _graphics?.DrawText(_consolasBold, 20f, _lawngreen, textOffsetX, statsYOffset, gameMemory.SegmentFormattedString); //261f
            }
            ...
            if (config.ShowDifficultyAdjustment)
            {
                ...
                _graphics?.DrawText(_consolasBold, 20f, _lawngreen, textOffsetX, statsYOffset, gameMemory.EndResults.RankScore.ToString()); //110f

                textOffsetX = 0f;
                _graphics?.DrawText(_consolasBold, 20f, _grey, config.PositionX + 15f, statsYOffset += 24, config.RankString); //178f
                textOffsetX += config.PositionX + 15f + GetStringSize(config.RankString) + 10f;
                _graphics?.DrawText(_consolasBold, 20f, _lawngreen, textOffsetX, statsYOffset, gameMemory.EndResults.Rank.ToString()); //261f
                textOffsetX += GetStringSize(gameMemory.EndResults.Rank.ToString()) + 10f;
            }
            ...
            foreach (GameEnemy enemy in gameMemory.EnemyHealth.Where(a => a.IsAlive).OrderBy(a => a.Percentage).ThenByDescending(a => a.CurrentHP))
                if (config.ShowHPBars)
                {
                    DrawEnemyHealthBar(ref xOffset, ref yOffset, enemy.Name, enemy.CurrentHP, enemy.MaximumHP, enemy.Percentage);
                }
                else
                {
                    _graphics.DrawText(_consolasBold, 20f, _white, xOffset + 10f, yOffset += 28f, string.Format("{0}{1} / {2} {3:P1}", enemy.Name, enemy.CurrentHP, enemy.MaximumHP, enemy.Percentage));
                }
        }
        ...
        private void DrawEnemyHealthBar(ref float xOffset, ref float yOffset, string name, float chealth, float mhealth, float percentage = 1f)
        {
            ...
            _graphics.DrawText(_consolasBold, 20f, _lightred, xOffset + 10f, yOffset - 2f, string.Format("{0}{1} / {2}", name, chealth, mhealth));
